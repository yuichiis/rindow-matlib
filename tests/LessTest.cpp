...
protected:
    virtual void test_matlib_less(
        int32_t m,int32_t n,float *a, int32_t ldA,float *x, int32_t incX)
    {
        rindow_matlib_s_less(m,n,a,ldA,x,incX);
    }
    virtual void test_matlib_less(
        int32_t m,int32_t n,double *a, int32_t ldA,double *x, int32_t incX)
    {
        rindow_matlib_d_less(m,n,a,ldA,x,incX);
    }
};
typedef ::testing::Types<float, double> TestTypes;
TYPED_TEST_SUITE(LessTest, TestTypes);

TYPED_TEST(LessTest, normal) {
    const int32_t M = 2;
    const int32_t N = 3;
    
    TypeParam X[N] =    {-2,1,2};
    TypeParam A[M][N] = {{-1,0,4}, {2,0,2}};
    this->test_matlib_less(
        M,              // int32_t m,
        N,              // int32_t n,
        (TypeParam *)A, // *a,
        N,              // int32_t ldA
    ...
    TypeParam R2[M][N] = {{0,1,0},{0,1,0}};
    EXPECT_THAT(R2, ContainerEq(A));
}
TYPED_TEST(LessTest, normal2) {
    const int32_t M = 3;
    const int32_t N = 2;
...
TYPED_TEST(LessTest, non_a_numbers) {
    const int32_t M = 4;
    const int32_t N = 1;
